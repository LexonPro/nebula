    ...
    <div className="flex h-[calc(100vh-5.5rem)] bg-background overflow-hidden text-sm w-full max-w-[1920px] mx-auto">
      ...
      <div className="flex-1 overflow-auto custom-scrollbar relative">
        <div className="w-full min-w-max flex flex-col">
          ...
          {/* Non-sticky Top Area (vertically non-sticky, horizontally sticky) */}
          <div className="flex bg-card">
            <div className="sticky left-0 z-20 w-64 md:w-72 shrink-0 bg-card border-r border-b border-border p-6 flex flex-col justify-between">
              <div>
          ...
          <div className="flex sticky top-0 z-30 bg-background border-b border-border shadow-sm">
            {/* Sticky Top-Left Corner for the Grid */}
            <div className="sticky left-0 z-40 w-64 md:w-72 shrink-0 bg-card border-r border-border flex flex-col">
            ...
            <div className="flex flex-col flex-1">
              ...
              <div className="flex h-10 bg-card border-b border-border">
                {daysInMonth.map((d, i) => {
                  ...
                  return (
                    <div key={i} className={`flex-1 min-w-[30px] border-r border-border flex flex-col items-center justify-center ${color.bg}`}>
          ...
          <div className="flex flex-col">
            ...
            {habits.map((habit, rowIndex) => (
              <div key={habit.id} className="flex group hover:bg-muted/50 transition-colors">
                {/* Sticky Habit Name */}
                <div className="sticky left-0 z-20 w-64 md:w-72 shrink-0 bg-card group-hover:bg-muted border-r border-b border-border px-4 py-2.5 flex items-center gap-3 transition-colors">
                ...
                <div className="flex flex-1">
                  {daysInMonth.map((d, i) => {
                    ...
                    return (
                      <div key={i} className={`flex-1 min-w-[30px] h-full border-r border-b border-border flex items-center justify-center p-1.5 transition-colors ${isCompleted ? color.bg : ''}`}>
                        <button
